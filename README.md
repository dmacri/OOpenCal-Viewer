...
```
~/BestSoftwareEver/OOpenCAL/models/MyModel/MyModelCell.h
```
Then set:
```
...
- **Simulation directories**: Each run starts from a directory opened via `File → Open Model Directory`. The directory contains `Header.txt`, generated data files, and optionally the model C++ header and compiled module. `Header.txt` defines grid dimensions, number of simulation steps, output file prefix, and node tiling. The `GENERAL` section provides values such as `number_of_columns`, `number_of_rows`, and `output_file_name`, while the `DISTRIBUTED` section describes how many nodes (`number_node_x`, `number_node_y`) partition the domain.
...
- **Model-specific loaders**: Each model defines its own `Element` type and parsing rules. Plugins register readers through `SceneWidgetVisualizerFactory`, so the same viewer can inspect multiple simulation formats. Load models dynamically through `Model → Load Plugin…`, place plugins in `./plugins/`, or supply them via the `--loadModel` command-line argument. 
...
```bash
# Load simulation directory with specific model
./QtVtkViewer /path/to/model/directory --startingModel=MyModel

# Generate video automatically
./QtVtkViewer /path/to/model/directory --generateMoviePath=/tmp/movie.ogv --exitAfterLastStep

# Generate image at specific step
./QtVtkViewer /path/to/model/directory --step=100 --generateImagePath=/tmp/step100.png

# Batch processing with silent mode
./QtVtkViewer /path/to/model/directory --generateMoviePath=/tmp/movie.ogv --exitAfterLastStep --silent
```
...
```cpp
#include <OOpenCAL/models/MyModel/MyModelCell.h>
````
work correctly.
...
- ✅ **Load via GUI** - File → Open Model Directory... (Ctrl+O)
...
**Simulation Directory Loading:**
- Load new simulation directories via **File → Open Model Directory** (Ctrl+O)
- Automatically clears scene and reloads with new parameters from `Header.txt`
- Updates GUI (steps, dimensions, directory contents) automatically
...
2. **Model** → Load Plugin... (optional, if the directory should use an existing plugin)
3. **File → Open Model Directory** → Select the directory containing `Header.txt`
